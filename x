...
-- Function to create 2D corner boxes around the target
    ...
    if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
        ...
        for _, partName in ipairs({"Head", "UpperTorso", "LowerTorso", "HumanoidRootPart"}) do
            ...
            if part and part:IsA("BasePart") then
                ...
                if onScreen then
                    box[partName] = screenPos
                end
            end
        end

        if next(box) then
            for partName, screenPos in pairs(box) do
                -- Create lines around the target
                if not BoxLibrary[partName] then
                    BoxLibrary[partName] = Drawing.new("Line")
                    BoxLibrary[partName].Visible = false
                    BoxLibrary[partName].Color = getRainbowColor()
                    BoxLibrary[partName].Transparency = 1
                    BoxLibrary[partName].Thickness = 2
                end

                local screenPosX = screenPos.X
                local screenPosY = screenPos.Y
                local size = 50  -- Size of the box (adjust as needed)
                local offset = 10 -- Space between the corners

                -- Draw box corners
                BoxLibrary[partName].From = Vector2.new(screenPosX - size - offset, screenPosY - size - offset)
                BoxLibrary[partName].To = Vector2.new(screenPosX + size + offset, screenPosY - size - offset)
                BoxLibrary[partName].Visible = true
            end
        end
    end
end
...
local function handleESP()
    if visualsActive and getgenv().Visuals.Enabled then
        if IsTargeting and TargetPlayer then
            drawBox(TargetPlayer)
            updateTracer()
        else
            for _, line in pairs(BoxLibrary) do
                line.Visible = false
            end
            if TracerLine then
                TracerLine.Visible = false
